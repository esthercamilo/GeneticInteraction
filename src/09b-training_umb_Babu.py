...
dic_complete = {}
output_nom = open(folder+'weka/Babu/umbalanced_nom.csv','w')
output_num = open(folder+'weka/Babu/umbalanced_num.csv','w')
for k in diccent.keys():
	try:
		dic_complete[k] = diccent[k] + dicneigh[k] + dicsp[k] + dicbabu[k]
		dic_completen[k] = diccent[k] + dicneigh[k] + dicsp[k] + dicnombabu[k]
		output_nom.write(','.join(dic_completen[k])+'\n')
		output_num.write(','.join(dic_complete[k])+'\n')
	except:
		pass
